...
| brand_type | Yes      | |
| model      | Yes      | |
| weight     | No       | Not extracted; calculated via weight allocation (FR31-FR36) |
...
* **FR12:** System can handle merged cells by unmerging
  ...
  - **Processing order (CRITICAL - must follow exactly):**
    ...
    5. **Propagate if needed:** Optionally call `propagate_string_values()` for bulk propagation, but merge-aware reading (step 4) is the primary mechanism
  - **Implementation Note:** The extraction code MUST handle merged cells explicitly. Simply unmerging is NOT sufficient - the extraction must either read from origins or propagate values first.
...
* **FR22:** System can extract total_nw (total net weight) directly from the nw column on the total row.
  ...
  - **Visible Precision (CRITICAL - differs from line-level nw):**
    ...
    - Examples:
      ...
      - Cell value `212.5` with format `General` → store as `Decimal("212.5")` (no format-based rounding)
...
* **FR26:** System cleans 'po_no' values by removing suffix starting from first "-" or "/" delimiter.
  - Examples: `2250600556-2.1` → `2250600556`, `PO12345/1` → `PO12345`
...
- **Example**: `45.23` → precision 2, `1.955` → precision 3, `100.0` → precision 2 (minimum)
...
* **FR30:** System validates packing weights sum BEFORE allocation, then detects decimal precision.
  ...
  - Detects base precision from `total_nw` value (minimum 2 decimals, maximum 6 decimals)
...
* **FR35:** Weight Allocation Error Messages:
  ...
  - `[ERR_044] Part '{part_no}' weight rounds to zero even at maximum 6 decimal precision`
...
* **FR60:** System displays real-time processing status to console (see **Console Output Format** section for detailed examples and exact formatting requirements)
...
* **FR66:** System entry point (`__main__.py`) must wire to batch processor and execute the full processing pipeline.
  ...
  - Returns appropriate exit codes (0=success, 1=failures, 2=config error)
